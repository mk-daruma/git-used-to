...
RSpec.describe "Api::V1::QuizRepositoryFiles", type: :request do
  ...
  describe "POST /create" do
    ...
    context "送られてきた配列内のファイル情報が一つの場合" do
      it "データ登録が成功すること" do
        expect { post api_v1_quiz_repository_files_path, params: params }.to change(QuizRepositoryFile, :count).by(+1)
    ...
    context "送られてきた配列内のファイル情報が複数の場合" do
      it "データ登録が成功すること" do
        expect { post api_v1_quiz_repository_files_path, params: mulch_params }.to change(QuizRepositoryFile, :count).by(+2)
        res = JSON.parse(response.body)
        expect(res["status"]).to eq("SUCCESS")
  ...
  describe "delete /destroy" do
    it "quiz_repository_fileデータの削除が成功すること" do
      expect { delete api_v1_quiz_repository_file_path(quiz_repository_file.id) }.to change(QuizRepositoryFile, :count).by(-1)
      res = JSON.parse(response.body)
      expect(res["status"]).to eq("SUCCESS")
      expect(res["message"]).to eq("Deleted the post")
      expect(res["data"]["id"]).to eq(quiz_repository_file.id)
      expect(response).to have_http_status(:success)
    end
  end
end
