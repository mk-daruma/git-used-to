  ...
  describe "GET /api/v1/users#user_ranking" do
    ...
    context "作成したクイズに関連するブックマーク数の合計が10を超えているuserの場合" do
      it "ブックマーク総数が多い順にuserのid/名前/プロフィール画像とブックマーク総数のデータが取得できること" do
        ...
        expect(res["user_ranking"][0]["create_user_introduction"]).to eq(rank_in_user2.user_self_introduction)
        expect(res["user_ranking"][0]["create_user_image"]).to eq(rank_in_user2.image.url)
        expect(res["user_ranking"][0]["create_user_title"]).to eq(rank_in_user2.nickname)
        ...
        expect(res["user_ranking"][1]["create_user_introduction"]).to eq(rank_in_user1.user_self_introduction)
        expect(res["user_ranking"][1]["create_user_image"]).to eq(rank_in_user1.image.url)
        expect(res["user_ranking"][1]["create_user_title"]).to eq(rank_in_user1.nickname)
